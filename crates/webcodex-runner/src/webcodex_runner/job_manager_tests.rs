...
fn assert_local_job_stdin_isolated(test_name: &str, validation: bool) {
    ...
    if std::env::var(FIXTURE_ENV).as_deref() == Ok(test_name) {
        ...
        let mut request: RunnerRequest = serde_json::from_value(json!({
            "request_id": "stdin-isolation-request",
            "client_id": "ws-client",
            "kind": "start_job",
            "job_id": "stdin-isolation-job",
            "cwd": root.path(),
            "command": probe,
            "timeout_secs": 5,
            "requested_by": "test",
            "created_at": 0,
            "job_context": test_job_context(root.path(), Vec::new()),
        }))
        .unwrap();
        if validation {
        ...
        let sink = RunnerSink::WebSocket {
            ...
            runner_instance_id: "stdin-isolation-instance".to_string(),
        };
        let manager = JobManager::new(1);
        manager.enqueue(
            sink,
